...
# .bin 파일들 압축
bin_files = list(Path(model_dir).glob("*.bin"))
total_original_size = 0
total_compressed_size = 0


for i, file_path in enumerate(bin_files, 1):
    print(f"\n[{i}/{len(bin_files)}] Compressing {file_path.name}...")
...
config_files = ['config.json', 'generation_config.json', 'pytorch_model.bin.index.json',
